...
public class RPGTalk : MonoBehaviour {
    ...
    private static RPGTalk instance;
    ...
    public static RPGTalk Instance
    {
        get
        {
            if(instance == null)
            {
                instance = (RPGTalk)FindObjectOfType<RPGTalk>();
            }
            return instance;
        }
    }
    ...
    /// <summary>
    /// A GameObject to show when the dialog is over.
    /// </summary>
    public GameObject EndDialogIcon;
	/// <summary>
    ...
    /// <param name="_callbackScript">Script to be called when the talk ends</param>
    /// <param name="_txtToParse">Function to be called when the talk ends</param>
    public void NewTalk(string _lineToStart, string _lineToBreak, TextAsset _txtToParse, UnityEvent callbacks)
    {
        lineToStart = _lineToStart;
        lineToBreak = _lineToBreak;
        txtToParse = _txtToParse;
    ...
    public void NewTalk(){
		...
		if (dialoger) {
			...
			if (shouldUsePhotos) {
                ...
                //if the speaker has no photo attached to his name dont show photo
                if (UIPhoto && !hasPhoto)
                {
                    UIPhoto.sprite = null;
	...
	void Update () {
		...
		if (textUI.enabled &&
			...
			if (cutscenePosition >= rpgtalkElements.Count && shouldStayOnScreen) {
				if( (lookForClick && ( passWithMouse && CheckForValidClick()) ) || (passWithInputButton != "" && Input.GetButtonDown(passWithInputButton)) )
                {
     ...
     //                   CancelInvoke ("blink");
			...
			if (
				enablePass && (
				(passWithMouse && CheckForValidClick() ) ||
	...
	void TextEnded(){
        ...
        if (enablePass)
        {
            ...
            //if we are are the end of the conversation we will show end conversation icon
            if (cutscenePosition + 1 > rpgtalkElements.Count)
            {
                if (EndDialogIcon)
                {
                    EndDialogIcon.SetActive(true);
                }
            }
            // Otherwise we show the continue arrow
            else
            {
                blink();
            }
	...
	public void MadeAChoice(int questionID, int choiceNumber){
        ...
        //if have an audio to play when a choice was made play it
        if (choiceAudio != null)
        {
            rpgAudioSorce.clip = choiceAudio;
            rpgAudioSorce.Play();
        }
    ...
    protected bool CheckForValidClick()
    {
        ...
        if (Input.GetMouseButtonDown(0) && EventSystem.current.IsPointerOverGameObject())
        {
            ...
            foreach (var result in results)
            {
                foreach (GameObject go in showWithDialog)
                {
                    if (result.gameObject.transform.root.gameObject == go)
                    {
                        return true;
                    }
                }
    ...
    protected IEnumerator RegisterToEvents()
    {
        yield return new WaitForEndOfFrame();

        Messenger.AddListener("OnPauseGame", OnPauseGame);
        Messenger.AddListener("OnUnpauseGame", OnUnpauseGame);
    }

    public void OnSceneLoaded(Scene scene, LoadSceneMode sceneMode)
    {
        //we cant to register to event but waiting until the end of the frame, becase the Messenger class cleans up also all events in OnSceneLoaded, and we dont know if it can be called after this one therefore deleting the listener
